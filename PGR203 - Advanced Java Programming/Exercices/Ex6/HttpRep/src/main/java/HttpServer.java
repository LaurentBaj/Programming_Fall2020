...
public class HttpServer {
    ...
    private void handleRequest(Socket clientSocket) throws IOException {
        ...
        if (questionPos != -1) {
            ...
            if (queryString.getParameter("status") != null) { // Look for status value
                statusCode = queryString.getParameter("status");
            }
            if (queryString.getParameter("body") != null) { // Look for body content
                body = queryString.getParameter("body");
            }
        } else if (!requestPath.equals("/echo")) {  // Look for file
            File file = new File(contentRoot, requestPath);
            if (!file.exists()) {   // Create new file if it does not exist
    ...
    public List<String> getProductNames() { // List of values stored in post
        return productNames;
    }
}